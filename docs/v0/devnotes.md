...
3. Build ingestion endpoint: upload/import docs ✓ (POST /ingest: extract → clean → chunk → embed → store; step3notes.md)
...
8. Build answer endpoint: send (question + retrieved chunks) to LLM (Gemini) → get response ✓ (POST /answer; step8notes.md)
9. Return response + citations (metadata for retrieved chunks) ✓ (included in /answer response)
